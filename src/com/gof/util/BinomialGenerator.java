...
public class BinomialGenerator {
     ...
     * @throws Exception if <tt>n*Math.min(p,1-p) &lt;= 0.0</tt>
     */
    public BinomialGenerator(int n, double p) throws Exception {
    	
  	   if(n*Math.min(p,1-p) <= 0.0) throw new Exception("error in BinomialGenerator => n*Math.min(p,1-p) <= 0.0 (n : " + n + ", p : " + p + ")");
    ...
    /**
     * Constructs a binomial distribution with given random generator.
     * Example: n=1, p=0.5.
     * @param n the number of trials (also known as <i>sample size</i>).
     * @param p the probability of success.
     ...
     * @throws Exception if <tt>n*Math.min(p,1-p) &lt;= 0.0</tt>
     */
    public BinomialGenerator(int n, double p, Random randomGenerator) throws Exception {
	...
	public double probability(int k) {
		...
		double num1 = 0;
		for(int i=1; i<=k; i++) num1 += Math.log(this.n+1-i);
		
		double num2 = 0;
		for(int i=1; i<=k; i++) num2 += Math.log(i);
		
		double logP = num1 - num2 + k * Math.log(this.p) + (this.n-k) * Math.log(1-this.p);
	...
	public static double probability(int k, int n, double p) {
		...
		double num1 = 0;
		for(int i=1; i<=k; i++) num1 += Math.log(n+1-i);
		
		double num2 = 0;
		for(int i=1; i<=k; i++) num2 += Math.log(i);
		
		double logP = num1 - num2 + k * Math.log(p) + (n-k) * Math.log(1-p);
  ...
  //	    ******************************************************************/
     ...
     * </tt>
     */
    private double stirlingCorrection(int k) {
