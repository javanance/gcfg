...
@Getter
@Setter
public abstract class DerivativesAbstractOld extends InstrumentAbstract {
...
//	protected double      paySpotPriceBase;
	...
	public DerivativesAbstractOld(Integer scenNum, Map<String, IrCurveHis> scenarioCurveHis) {		
		super();
		this.scenNum          = scenNum;
		this.scenarioCurveHis = scenarioCurveHis;		
	}
	
	public DerivativesAbstractOld() {		
		super();
		this.scenNum          = 0;
	...
	/**
	 * TODO: 예전에 이렇게 코딩한 이유를 파악해야 한다. 물론 수취/지급 레그별로 다르기는 할거다. 단, 외화자산은 아래의 최초 if조건에서 벗어난다.(DEF_CURRENCY 가 아님)
	 * isRecIrCalibration은 FALSE(0)상태로 최초에 초기화가 된다. 
	 */
	private void setRecScenarioCurveHis() {
		
		if(this.recIrCurveId != null && this.recCurrency.equals(DEF_CURRENCY)) {		
			log.info("rec in set IR: {}", recIrCurveId);
		...
		log.info("rec dis rate : {},{},{}", recDiscountSpread, recMatTermIntRate);
...
//		if(this.payIrCurveId != null && this.payCurrency.equals(DEF_CURRENCY) && this.isRecIrCalibration == INTEGER_FALSE) {
			...
			log.info("pay in setIr: {},{}", payIrCurveId, isRecIrCalibration);
			this.toRealScale = this.isRealNumber ? 1.0 : TO_REAL_INT_RATE;
		...
		log.info("pay dis rate : {},{},{}", payDiscountSpread, payMatTermIntRate);
	...
	/**
	 * NET_LEG는 CF의 방향성이 단조/증가가 아니므로, 수렴에 빈번하게 실패하여 [REC-PAY]로 의제처리하함
	 */    
