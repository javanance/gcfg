...
//	}
	 ...
	 * 
	 * this.currency.equals(DEFAULT_CURRENCY) --> 기본 화폐코드를 맞추려고 했는데 다르게 적용됐네요.. 아마도 코드에 두 가지 타입이 있어서 그런 것 같습니다...(하나는? 기본화폐?)
	 * isIrCalibration이 FALSE(0)일때만 초기화가 됩니다.
	 * setScenarioCurveHis를 호출하는 시점에 implied spread를 적용하고 그 이후에는 업데이트하지 않습니다. 아마도 standard를 맞추려고 하는 것 같습니다?
	 * 필요하다면 다른 클래스에서 setter를 통해 처리하는 것도 고려해볼까요? 기본 설정은 문제가 없지만...
	 * setIrCurveHis에서 matTermIntRate에 대한 추가 처리가 필요한가요? 예를 들어 초기값이 0.0이기 때문에 계산에 문제가 있을 수 있습니다.
	 */	
...
//		log.info(" in evaluation BondAbs end2: {},{}", maturityDate, adjMaturityDate);
	 ...
	 * �����ȯ���� �����ϴ� �༮�鿡 ���ؼ��� �� �޼ҵ� ������ ������ؾ��ҵ���
	 * 
	 * MAIN에서 호출되는 함수이며 이 클래스 안에서만 호출할 수 있음
	 * adjMaturityDate가 생성되지 않은 초기화 상태에서는, 이 함수가 제대로 동작하지 않음, calibration 과정에서 impliedMaturity가 클래스에 세팅되어야 함
	 * else 블록에서는 이와 같은 상황을 처리하도록 코드를 작성해야 함.
	 * 환경설정을 변경하는 모듈들에 대해서도 이 메서드를 호출하도록 구성해야 함.
	 */
	@Override
	...
	/**
	 * Macaulay Duration을 계산하는 데 있어서 paymentTerm을 사용하는 것이 맞는지 확인 필요... compoundPeriod가 아닌지 확인...
	 */
     ...
     * 
     * price와 currency를 받아서...getPresentValue와 몇을 처리하는 반영을 하는 것이 아니다. 
     * 여기서 pvPositive와 정대를 호출하는 한 번의 메서드를 별도로 하는 방법입니다. 
     * 이를 활용하기 위한 팁은 활용이 하는 다른 기능을 넣으셔야 한다.
     */
    ...
    /**
		TODO: this.isIrCalibration이 calibration되었고 IR shock 전송하는 경우 INTEGER_TRUE의 속성을 설정한다.
		또한, 이 메서드 내 객체의 컬렉션으로 사용하기 위해 total MAP에 저장된 정보와 일치하는 IR Shock이 올바르게 전송된다.
		따라서 FX전송으로부터 최신환율정보를 calibration하는 절차를 따라서, 이 정보를 업데이트하는 작업을 수행해야 한다.
		기본적으로 IrCalibration이 사용되는(StanDard 전송을) 활용하여 이 메서드 내 컬렉션이 올바르게 전송된다.
     */
...
//    		log.info("i :{} ,calc spread :{}",i, spreadNew);
    ...
    /**
     * this.result.setValue(Math.floor(this.interestPayoffAmount[i] * fxRatio)) -> floor 함수를 추가할 필요가 있음 (1원 미만의 소수점 처리를 위해 필요)
     */	
	public List<KicsAssetResult> getValuation(boolean currencyType) throws Exception {		
		
//		log.info("At getValuation : {}", currencyType);
		// time bucket별 현금흐름 생성 
		evaluateCashflow();		
...
//    		log.info("in financialElements In Bond Abs : {},{}", fe, financialElements.size());
    		...
    		switch(fe.intValue()) {
			    case FE_PRINCIPAL_CF: {
			    	log.debug("calc FE : {} FE_PRINCIPAL_CF ", fe );
			    ...
			    case FE_INTEREST_CF: {			
			    	log.debug("calc FE : {} FE_INTEREST_CF", fe );
			    	for(int i=0; i<payoffDate.length; i++) {			    		
			    		this.result.setResultDate(TimeUtil.dateToString(payoffDate[i]));
			    ...
			    case FE_YIELD_TO_MATURITY: {			
			    	log.debug("calc FE : {} FE_YIELD_TO_MATURITY", fe );
			    ...
			    case FE_PV_DIRTY: {			    	
			    	log.debug("calc FE : {} FE_PV_DIRTY", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));
			    ...
			    case FE_ACCRUED_INTEREST: {				
			    	log.debug("calc FE : {} FE_ACCRUED_INTEREST", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));    	
			    ...
			    case FE_PV_CLEAN: {					
			    	log.debug("calc FE : {} FE_PV_CLEAN", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));
			    ...
			    case FE_EFFECTIVE_MATURITY: {		
			    	log.debug("calc FE : {} FE_EFFECTIVE_MATURITY", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));    	
			    ...
			    case FE_EFFECTIVE_DURATION: {						    							    	
			    	log.debug("calc FE : {} FE_EFFECTIVE_DURATION", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));    	
			    ...
			    case FE_MODIFIED_DURATION: {			
			    	log.debug("calc FE : {} FE_MODIFIED_DURATION", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));    	
			    ...
			    case FE_BS_AMOUNT: {		
			    	log.debug("calc FE : {} FE_BS_AMOUNT", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));			    	
			    ...
			    case FE_ACCRUED_INT_BS: {		
			    	log.debug("calc FE : {} FE_ACCRUED_INT_BS", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));			    	
			    ...
			    case FE_IRATE_EXPOSURE: {				
			    	log.debug("calc FE : {} FE_IRATE_EXPOSURE", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));
			    	this.result.setValue(Math.floor(getPresentValueClean() * fxRatio));						    	
			    	cflist.add(cloneEntity(this.result));
			    	break;
			    ...
			    case FE_FXRT_EXPOSURE: {		
			    	log.debug("calc FE : {} FE_FXRT_EXPOSURE", fe );
			    ...
			    case FE_STOCK_EXPOSURE: {			
			    	log.debug("calc FE : {} FE_STOCK_EXPOSURE", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));
			    ...
			    case FE_IMPLIED_SPREAD: {
			    	log.debug("calc FE : {} FE_IMPLIED_SPREAD", fe );
			    	this.result.setResultDate(TimeUtil.dateToString(this.baseDate));
			    ...
			    case FE_IMPLIED_MATURITY: {		
			    	log.debug("calc FE : {} FE_IMPLIED_MATURITY", fe );
